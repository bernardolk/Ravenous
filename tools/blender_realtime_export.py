...
class OBJECT_PT_realtime_export(ObjectButtonsPanel):
    ...
    def draw(self, context):        
        ...
        row.prop(obj, 'prop_path', icon='FILE_FOLDER')
...
def simple_export(obj, rec_path):    
    ...
    if '.obj' not in rec_path:
        rec_path = rec_path + '.obj' 

    obj_text = open(rec_path, 'w')
    
    lines = []
    lines.append('#num of vertices ' + str(len(v_list)) + '\n')    
...
def initialize():
    ...
    bpy.types.Object.prop_path = bpy.props.StringProperty(name="path", description="Recording Folder", subtype='FILE_PATH')
